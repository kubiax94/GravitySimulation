...
namespace {
	struct gl_test_context {
		...
		gl_test_context() {
			...
			window = glfwCreateWindow(64, 64, "RenderPipelinePerfHeavyTest", nullptr, nullptr);
...
{
	TEST_CLASS(RenderPipelinePerfHeavyTest)
	{
	public:
		TEST_METHOD(RenderPipelineFlushInstancedHeavy)
		{
			constexpr int RendererCount = 25000;
			constexpr int FrameCount = 160;

			gl_test_context context;
			const auto shader_dir = std::filesystem::temp_directory_path() / "GravitySimulation" / "RenderPipelinePerfHeavyTest";
...
void main()
{
    FragColor = vec4(1.0, 1.0, 1.0, 1.0);
			...
			Assert::IsTrue(batch_shader->is_vaild(), L"shader failed to load");

			MeshData mesh_data = create_mesh_data();
			auto batch_mesh = std::make_unique<Mesh>(mesh_data);
			Assert::IsTrue(batch_mesh->is_vaild(), L"mesh failed to initialize");
			...
			for (int i = 0; i < RendererCount; ++i)
			{
				...
				node->set_position(static_cast<float>(i % 250), static_cast<float>((i / 250) % 100), static_cast<float>(i % 31));
			...
			for (int frame = 0; frame < FrameCount; ++frame)
			{
				for (int i = 0; i < RendererCount; ++i)
				{
					nodes[i]->set_position(static_cast<float>((i + frame) % 250), static_cast<float>(((i / 250) + frame) % 100), static_cast<float>((i + frame * 3) % 31));
				}
			...
			Assert::IsTrue(sink >= 0.0f || renderers.empty());
		}
	};
}
